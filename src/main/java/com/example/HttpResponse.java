...
public abstract class HttpResponse implements AutoCloseable {
     ...
     *
     * <p>
     * Returns a stream of the response's body content. Emissions may occur on the Netty EventLoop threads which are
     * shared across channels and should not be blocked. Blocking should be avoided as much as possible/practical in
     * reactive programming but if you do use methods like {@code blockingSubscribe} or {@code blockingGet} on the
     * stream then be sure to use {@code subscribeOn} and {@code observeOn} before the blocking call. For example:
     *
     * <pre>
     * {@code
     *   response.body()
     ...
     * </pre>
     * <p>
     * The above code is a simplistic example and would probably run fine without the `subscribeOn` and `observeOn` but
     * should be considered a template for more complex situations.
     *
